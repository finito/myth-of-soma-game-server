...
IMPLEMENT_DYNAMIC(CStatExpTableDSet, CRecordset)

CStatExpTableDSet::CStatExpTableDSet(CDatabase* pdb)
	: CRecordset(pdb)
{
	//{{AFX_FIELD_INIT(CStatExpTableDSet)
	m_sid;
	m_slevel;
	m_str;
	m_dex;
...
CString CStatExpTableDSet::GetDefaultConnect()
{
	return g_strDatabaseConnectionString;
}

CString CStatExpTableDSet::GetDefaultSQL()
{
	return _T("[dbo].[d_tb_inc]");
}

void CStatExpTableDSet::DoFieldExchange(CFieldExchange* pFX)
{
	//{{AFX_FIELD_MAP(CStatExpTableDSet)
...
#ifdef _DEBUG
void CStatExpTableDSet::AssertValid() const
{
	CRecordset::AssertValid();
}

void CStatExpTableDSet::Dump(CDumpContext& dc) const
{
	CRecordset::Dump(dc);
}
#endif //_DEBUG
