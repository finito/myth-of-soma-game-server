...
{
	if (InterlockedCompareExchangePointer((PVOID*)&m_lMapUse, (PVOID)1, (PVOID)0) == (PVOID*)0)
	{
		m_vMapSize.cx = x;
		m_vMapSize.cy = y;
		m_pMap = map;
...
NODE *CPathFind::FindPath(CDC& dc, int cellx, int celly, int start_x, int start_y, int dest_x, int dest_y)
{
	...
	t_node = (NODE *)calloc(1, sizeof(NODE));
	t_node->g = 0;
	t_node->h = (int)sqrt((float)(start_x-dest_x)*(start_x-dest_x) + (start_y-dest_y)*(start_y-dest_y));
	t_node->f = t_node->g + t_node->h;
	t_node->x = start_x;
	t_node->y = start_y;
