...
IMPLEMENT_DYNAMIC(CSkillExpTableDSet, CRecordset)

CSkillExpTableDSet::CSkillExpTableDSet(CDatabase* pdb)
	: CRecordset(pdb)
{
	//{{AFX_FIELD_INIT(CSkillExpTableDSet)
	m_sid;
	m_slevel;
	m_axe;
	m_bow;
...
CString CSkillExpTableDSet::GetDefaultConnect()
{
	return g_strDatabaseConnectionString;
}

CString CSkillExpTableDSet::GetDefaultSQL()
{
	return _T("[dbo].[d_tb_inc_exp]");
}

void CSkillExpTableDSet::DoFieldExchange(CFieldExchange* pFX)
{
	//{{AFX_FIELD_MAP(CSkillExpTableDSet)
...
#ifdef _DEBUG
void CSkillExpTableDSet::AssertValid() const
{
	CRecordset::AssertValid();
}

void CSkillExpTableDSet::Dump(CDumpContext& dc) const
{
	CRecordset::Dump(dc);
}
#endif //_DEBUG
