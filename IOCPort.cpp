...
DWORD WINAPI AcceptThread(LPVOID lp)
{
	CIOCPort* pIocport = reinterpret_cast<CIOCPort*>(lp);
	...
	while (1)
	{
		DWORD wait_return = WaitForSingleObject(pIocport->m_hListenEvent, INFINITE);
		...
		if (network_event.lNetworkEvents &FD_ACCEPT)
		{
			if (network_event.iErrorCode[FD_ACCEPT_BIT] == 0)
			{
				...
				CIOCPSocket2* pSocket = pIocport->GetIOCPSocket(sid);
				if (!pSocket)
				{
...
DWORD WINAPI ReceiveWorkerThread(LPVOID lp)
{
	...
	while (1)
	{
		...
		if (b || pOvl)
		{
			if (b)
			{
				if (WorkIndex > static_cast<DWORD>(pIocport->m_SocketArraySize))
				{
					goto loop_pass;
				}

				CIOCPSocket2* pSocket = reinterpret_cast<CIOCPSocket2*>(pIocport->m_SockArray[WorkIndex]);
				if (!pSocket)
				{
					goto loop_pass;
				}
			...
			else
			{
				if (pOvl)
				{
					...
					CIOCPSocket2* = reinterpret_cast<CIOCPSocket2*>(pIocport->m_SockArray[WorkIndex]);
					if (!pSocket)
					{
						goto loop_pass;
					}
...
DWORD WINAPI ClientWorkerThread(LPVOID lp)
{
	...
	while (1)
	{
		...
		if (b || pOvl)
		{
			if (b)
			{
				if (WorkIndex > static_cast<DWORD>(pIocport->m_ClientSockSize))
				{
					goto loop_pass;
				}

				CIOCPSocket2* pSocket =  reinterpret_cast<CIOCPSocket2*>(pIocport->m_ClientSockArray[WorkIndex]);
				if (!pSocket)
				{
					goto loop_pass;
				}
				...
				case OVL_RECEIVE:
					...
					if (!nbytes)
					{
						TRACE("Socket Closed By 0 Byte Notify\n");
						pSocket->CloseProcess();
						pIocport->RidIOCPSocket(pSocket->GetSocketID(), pSocket);
						LeaveCriticalSection(&g_critical);
						break;
					...
					pSocket->ReceivedData(static_cast<int>(nbytes));
					pSocket->Receive();
				...
				if (pOvl)
				{
					...
					TRACE("Socket Closed By Abnormal Termination\n");
					pSocket->CloseProcess();
					pIocport->RidIOCPSocket(pSocket->GetSocketID(), pSocket);
...
BOOL CIOCPort::Listen(int port)
{
	...
	int socklen = SOCKET_BUFF_SIZE * 4;
	setsockopt(m_ListenSocket, SOL_SOCKET, SO_RCVBUF, (char*)&socklen, sizeof(socklen));

	int len = sizeof(socklen);
	int err = getsockopt(m_ListenSocket, SOL_SOCKET, SO_RCVBUF, (char*)&socklen, &len);
	if (err == SOCKET_ERROR)
	{
...
void CIOCPort::PutOldSid(int sid)
{
	...
	Iter = std::find(m_SidList.begin(), m_SidList.end(), sid);
...
void CIOCPort::CreateReceiveWorkerThread(int workernum)
{
	...
	for (int i = 0; i < static_cast<int>(m_dwNumberOfWorkers); i++)
	{
...
CIOCPSocket2* CIOCPort::GetIOCPSocket(int index)
{
	...
	CIOCPSocket2* pIOCPSock = reinterpret_cast<CIOCPSocket2*>(m_SockArrayInActive[index]);
